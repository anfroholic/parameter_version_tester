...
if is_wasm:
    print('creating fake websocket server, we are wasm')    
else:
...
class Client(WebSocketClient):
    ...
    def open_file(self, filename):
        ...
        _filename = 'sd/temp' if filename[:3] == b'sd/' else 'temp'
        self.file = open(_filename, 'wb')
    
    def close_file(self):
        print('closing fiel:', self.filename)
        self.file.close()
        self.file = None
        if self.filename[:3] == "sd/":
            os.rename('sd/temp', self.filename)    
        else:
            ...
            # -------
                ...
                with open(data, 'r') as f:
                    try:
                        ...
                        self.connection.write(f'to_file_editor,UnicodeError,UnicodeError')
                return
                ...
                self.connection.write(f'listdir,{json.dumps(get_directory_structure('/'))}')
                return
            # -------
