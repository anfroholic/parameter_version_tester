...
adr:0 -> EMCY
adr:1 -> NETWORK (high priority)
adr:2 -> ZORG (high priority)
adr:1 -> NETWORK (low priority)
adr:254-> NETWORK (low priority)
"""
...
class CANBus:
    ...
    def send(self, load, h):
        try:
            ...
            adr, pid, msg_type = self.header.unpack(h)
            param = self.iris.p[pid]
            info = f"""
***** CAN ERROR *****
name: {param.name if hasattr(param, 'name') else "no_name"}
pid: {pid}
type: {type(param)}
load: {load}
can 29bit arbitration id: {h}
send adr: {adr}
...
"""
    ...
    def debug(self, msg):
        "send debugstrings to zorg"
        # TODO change this to generator 
        pid = self.header.adr + 1000  # this is this device's coms lvl
        if isinstance(msg, str):
            msg = msg.encode()
        # post = post + struct.pack("B", len(post)) + b'\x03'
        for i in range(0, len(msg), 8):
            segment = msg[i:i+8]
            self.iris.send(pid=pid, 
                           load=segment, 
                           type=1, 
                           adr=ZORG_CHANNEL_LOW,
                           )
        self.iris.send(pid=pid, # send closing frame
                           load=b'', 
                           type=1, 
                           adr=ZORG_CHANNEL_LOW,
                           )
