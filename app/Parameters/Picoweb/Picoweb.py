...
import floe
is_wasm = floe.implementation.wasm
if not is_wasm:
    # we are running in the browser and do not actually run anything
    import micropython
    import utime
    import uio
    import ure as re
...
class WebApp:
    ...
    def _handle(self, reader, writer):
        ...
        try:
            ...
            if request_line == b"":
                if self.debug >= 0:
                    print(f"{reader}: EOF on request start")
            ...
            if self.debug >= 0:
                print(e, "%.3f %s %s %r" % (utime.time(), req, writer, e))
        ...
        if __debug__ and self.debug > 1:
            print("%.3f %s Finished processing request", utime.time(), req)
...
def create_links(routes):
    return ("<br>").join(f'<a href="{route[0]}">{route[0]}</a><br>' for route in routes)
...
class Picoweb(Parameter):
    ...
    def update(self):
        super().update()
        if self.camera:
            self.routes.extend(self.camera.setup_picoweb(start_response))        
        
    def boot(self):
        ip_address = self.iris.core.wlan.ifconfig()[0]
        self.app = WebApp(__name__, self.routes)  
        self.app.run(debug=1, port=80, host=f"{ip_address}")
        
    # HTTP Response
    def index(self, req, resp):
        
        yield from resp.awrite(self._index())

    def _index(self):
        def links():
            return "<br>".join(f'<a href="{route[0]}">{route[0]}</a>' for route in self.routes)
        return f"""
HTTP/1.0 200 OK\r\n
<html>
  <head>
    <title>ESP32 Picoweb</title>
  </head>
  <body>
    <h1>Picoweb on ESP32</h1>
    <h3>running on floe by evezor</h3>
    <a href="https://floe.evezor.com">floe.evezor.com</a>
    <br><br><br>
    Available Routes
    <br>
    {links()}
