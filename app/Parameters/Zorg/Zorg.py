...
class Zorg:
    ...
    def parsemyblobs(self, blobs):
        blobmap = {}
        num_pids = len(blobs) / 3
        for i in range(num_pids):    
            pid, blob = struct.unpack("HB", blobs[i*3: (i*3)+3])
            blobmap[pid] = blob
        print(blobmap)
    ...
    def post(self, msg: bytearray):
        if self.posts:
            self.posts = self.posts + b'<br>' + msg
        else:
            self.posts = msg
        if self.post not in self.changes:
            self.changes.append(self._post)

    def terminal(self, load, adr):
        # catch packets and collate them to be sent to terminal
        # we will also 
        end = False
        if load == b'': # this is end of text char
            end = True
        ...
        if end:
            this_msg = self.terms[adr]
            if b':' in this_msg:
                cmd, msg = this_msg.split[b':', 1]
                if cmd in self.remote_cmds:
                    self.remote_cmds(msg)
            post = f"{adr}: {self.terms[adr]}"
            self.terms[adr] = b''
            self.post(post)
