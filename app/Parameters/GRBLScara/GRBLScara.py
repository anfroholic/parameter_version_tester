...
class GRBLScara(GRBL):
    def __init__(self, *, theta_encoder, phi_encoder, kinematics, name: str, iris: Iris, UART: FP, hbt: int = 1000, x: FP | None, y: FP | None, z: FP | None, a: FP | None, b: FP | None, c: FP | None, webserver_output: bool = False, **k):
        ...
        new_funcs = {
            'home_x': lambda *_: self.home('t'),  # self.run(self._home_theta),
            'home_y': lambda *_: self.home('p'),  # self.run(self._home_phi),
    ...
    def home(self, axis=None):
        if not axis:
            ...
            self.send_g(f'G10 L20 P1 X{self.theta_encoder.state}')
            # self.run(self._home_theta())
        elif axis == 'p':
            self.send_g(f'G10 L20 P1 Y{self.phi_encoder.state}')
    ...
    def parse_status(self, msg: str) -> None:
        ...
        msg = msg.strip('<>').split('|')
        self.status['limits'] = ""
        for idx, block in enumerate(msg):
            if idx == 0:
                self.status['state'] = block
            elif idx == 1:
                mpos = block[5:].split(',')  # remove Mpos:
            elif block[:2] == 'Pn':
                self.status['limits'] = block[3:]
